...
//根据选择的学期，获取所有的课程
export async function getCourseByTerm(term: string): Promise<{ id: number; name: string; }[]> {
  var response = await axios.post(host + "/getCourseByTerm", { term: term });
  return response.data as { id: number; name: string; }[];
}
//根据选择的学期和课程，获取所有的章节
export async function getParaByCourse(term: string, courseId: number): Promise<Para> {
  var response = await axios.post(host + "/getParaByCourse", { term: term, courseId: courseId });
...
export async function createNewText(
  ...
  return "<h1>444</h1>上了飞机啊收到了封建士大夫";
}
//获取所有的教案
...
export type Para={value:string,label:string,children:{value:string,label:string}[]}[]